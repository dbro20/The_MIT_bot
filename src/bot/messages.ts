import { DailyQuestion } from '../database/queries';
import { convertDBTimestampToTimezone } from '../utils/timezone';
  ...
  if (question.morning_question_sent_at) {
    ...
    if (question.morning_response) {
      ...
      if (question.morning_response_received_at) {
        const time = convertDBTimestampToTimezone(question.morning_response_received_at).toFormat(
          'h:mm a'
        );
  ...
  if (question.evening_question_sent_at) {
    ...
    if (question.evening_response) {
      ...
      if (question.evening_response_received_at) {
        const time = convertDBTimestampToTimezone(question.evening_response_received_at).toFormat(
          'h:mm a'
        );
        message += `🕐 Answered at: ${time}\n`;
      }
...
${stats.completionRate >= 80 ? '🎉 Great job! Keep up the excellent work!' : stats.completionRate >= 50 ? '👍 You\'re doing well! Try to be more consistent.' : '💪 Keep going! Consistency is key.'}`;
}
